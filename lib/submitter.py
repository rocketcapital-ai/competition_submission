...
class Submitter:
    """
    Submitter client for interacting with the Yiedl competitions.
    """
    ...
    def stake_and_submit(self, amount: Decimal | float | int, file_name: str,
        ...
        """
        Submits a new prediction or
        updates an existing prediction along with a stake amount.
    ...
    def withdraw(self, gas_price_in_gwei=None):
        """
        Withdraws submission then withdraws all stake.
        @param gas_price_in_gwei: (optional) Defaults to the "fast" gas price from
        polygonscan.com/gastracker. Otherwise an explicit gwei value can be stated here,
        or one of the three GasPriceMode modes.
        :return: True if completed successfully.
